...
@Dao public interface ContactDao {
  ...
  @Query("SELECT * FROM contacts_info WHERE id = :id") Maybe<Contact> getContactById(int id);
  ...
  @Insert void insertContact(Contact contact);

  @Query("UPDATE contacts_info SET is_log = 1 WHERE reminder_time = :time") void updateContactAsLog(
      long time);
